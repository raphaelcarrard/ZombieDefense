...
public class GameManager : MonoBehaviour
{
    ...
    public Text upgradePrice, increasePrice;
    public GameObject button1, button2;
    private int priceUp, priceIn;
    public Image upgradeIcon, increaseIcon;
    ...
    public GameObject[] gunvalue;

    public void LoadShop(){
        for(int i = 0; i < gunvalue.Length; i++){
            if(PlayerPrefs.GetInt("totalgun")==i){
                for(int j = 0; j < i; j++){
                    buyBtn[j].SetActive(false);
                    gunvalue[j].SetActive(false);
                }
                return;
            }
        }
    ...
    public void LoadUpgrade(){
        ...
        if(PlayerPrefs.GetInt("increase") >= 1){
            increaseIcon.sprite = increaseIconSpr[PlayerPrefs.GetInt("upgrade") - 1];
        }
    }
